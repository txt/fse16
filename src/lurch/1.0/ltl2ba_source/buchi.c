...
int simplify_btrans() /* simplifies the transitions */
{
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "\nSimplification of the Buchi automaton - transitions: %.2fs %i transitions removed\n", 
...
int simplify_bstates() /* eliminates redundant states */
{
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "\nSimplification of the Buchi automaton - states: %.2fs %i states removed\n", 
	   ((float)(t_fin.tms_utime - t_debut.tms_utime))/100, changed);
  }
...
{/* generates a Buchi automaton from the generalized Buchi automaton */
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "\nBuilding of the Buchi automaton : %.2fs %i states, %i transitions\n", 
